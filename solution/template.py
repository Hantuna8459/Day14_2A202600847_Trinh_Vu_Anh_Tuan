...
class RAGASEvaluator:
    ...
    def evaluate_faithfulness(self, answer: str, context: str) -> float:
        ...
        return min(max(len(overlap) / len(answer_tokens), 0.0), 1.0)
    ...
    def evaluate_relevance(self, answer: str, question: str) -> float:
        ...
        return min(max(len(overlap) / len(question_tokens), 0.0), 1.0)
    ...
    def evaluate_completeness(self, answer: str, expected: str) -> float:
        ...
        return min(max(len(overlap) / len(expected_tokens), 0.0), 1.0)
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            proportion = len(overlap) / len(expected_tokens)
            relevant_flags.append(proportion >= relevance_threshold)
        ...
        for idx, is_relevant in enumerate(relevant_flags, start=1):
            if is_relevant:
                relevant_seen += 1
                accumulated += relevant_seen / idx

        return min(max(accumulated / relevant_count, 0.0), 1.0)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            f"Score the following answer based on the rubric.\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}\n"
            f"Return a JSON object with a 'scores' dictionary of values between 0 and 1 and a 'reasoning' explanation."
        )
        ...
        try:
            ...
            for criterion, value in scores.items():
                ...
                if isinstance(value, (int, float)):
                    normalized_scores[str(criterion)] = min(max(float(value), 0.0), 1.0)
                else:
                    normalized_scores[str(criterion)] = 0.5
            return {
                "scores": normalized_scores or {k: 0.5 for k in rubric.keys()},
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if averages:
            leniency_bias = sum(averages) / len(averages) > 0.8
            severity_bias = sum(averages) / len(averages) < 0.3
            if len(averages) > 1 and averages[0] > max(averages[1:]):
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        report["avg_faithfulness"] = sum(r.faithfulness for r in results) / len(results)
        report["avg_relevance"] = sum(r.relevance for r in results) / len(results)
        report["avg_completeness"] = sum(r.completeness for r in results) / len(results)
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, failure in enumerate(failures, start=1):
            ...
            suggestion = suggestions[idx - 1] if idx - 1 < len(suggestions) else "Review pipeline and add more detailed investigation."
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0) > 0:
            suggestions.append(
                "Add grounding constraints and evidence checks to reduce hallucination."
            )
        if categories.get("irrelevant", 0) > 0:
            suggestions.append(
                "Clarify prompts and include explicit question intent to improve relevance."
            )
        if categories.get("incomplete", 0) > 0:
            suggestions.append(
                "Expand retrieved context coverage or ask the model to include all expected points."
            )
        if categories.get("off_topic", 0) > 0:
            suggestions.append(
                "Add intent alignment and topic filters to keep answers on subject."
            )

        # Always add some general improvement ideas if we have fewer than 3 suggestions
        if len(suggestions) < 3:
            suggestions.extend([
                "Implement automated failure clustering to target the most frequent error types.",
                "Calibrate judge prompts and rubric instructions to reduce scoring bias.",
            ])
